...
func TestSeedLeaves(t *testing.T) {
	tree_params, err := common.GetTreeParams(common.RSDP_1_SMALL)
	if err != nil {
		t.Errorf("Error: %s", err)

	}
	proto_params, err := common.GetProtocolConfig(common.RSDP_1_SMALL)
	if err != nil {
		t.Errorf("Error: %s", err)
	}
...
func TestFastSeedLeaves(t *testing.T) {
	tree_params, err := common.GetTreeParams(common.RSDP_1_FAST)
	if err != nil {
		t.Errorf("Error: %s", err)

	}
	proto_params, err := common.GetProtocolConfig(common.RSDP_1_FAST)
	if err != nil {
		t.Errorf("Error: %s", err)
	}
	seed := make([]byte, 32)
...
func TestIntegration(t *testing.T) {
	...
	for _, config := range configs {
		for xyz := 0; xyz < 100; xyz++ {
			tree_params, err := common.GetTreeParams(config)
			...
			proto_params, err := common.GetProtocolConfig(config)
			if err != nil {
				t.Errorf("Error: %s", err)
			}
